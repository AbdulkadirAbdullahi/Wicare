...
let ProfiledetailPage = class ProfiledetailPage {
    ...
    choosecover(event) {
        let alert3 = this.alertCtrl.create({
            title: '<font color="green"><b> </b> Cover image is only for hardcopy photoalbum and will be removed later, continue ? </font>',
            cssClass: "alert",
            buttons: [
                {
                    text: 'yes',
                    role: 'cancel',
                    handler: data => {
                        if (event.target.files[0].size < 503110) {
                            ...
                            let alert = this.alertCtrl.create({
                                title: '<font color="green"><b> </b>Cover image should be less then 501KB </font>',
                                cssClass: "alert",
                                buttons: [
                                    {
                                        text: 'Ok',
    ...
    async uploadcover() {
        ...
        if (this.selectedcover) {
            const filePath = `${this.basePath2}/${this.userData.name.trim() + this.userData.lga}`;
            this.filepath2 = filePath; // inorder to get the path to delete file
            this.task = this.storage.upload(filePath, this.selectedcover);
            this.progressValue = this.task.percentageChanges(); // <<<<< Percentage of uploading 
            (await this.task).ref.getDownloadURL().then(url => {
                this.downloadableURL = url;
                this.userData.coverurl = this.downloadableURL;
                try {
                    this.db.object('photoalbum/users/' + this.userId).update(this.userData)
                        .then(() => {
                        let alert2 = this.alertCtrl.create({
                            title: '<font color="green"><b> </b>Cover photo added successfully  </font>',
                            cssClass: "alert",
                            buttons: [
                                {
                                    text: 'Ok',
    ...
    dropcover(coverurl) {
        ...
        loading.present();
        this.storage.storage.refFromURL(coverurl).delete().then(async () => {
            this.user.coverurl = "";
...
MyApp = __decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["m" /* Component */])({template:/*ion-inline-start:"C:\Users\Abdulqadir\Desktop\Ionic_Mobile_App_Builder_v18.12.10\ionic2\PHOTO_ALBUM - with cover img\src\app\app.html"*/' <ion-nav #myNav  [root]="rootPage"></ion-nav>\n\n  \n\n\n\n  <!-- Disable swipe-to-go-back because it\'s poor UX to combine STGB with side menus -->\n\n\n\n'/*ion-inline-end:"C:\Users\Abdulqadir\Desktop\Ionic_Mobile_App_Builder_v18.12.10\ionic2\PHOTO_ALBUM - with cover img\src\app\app.html"*/
    }),
    __metadata("design:paramtypes", [__WEBPACK_IMPORTED_MODULE_1_ionic_angular__["a" /* AlertController */], __WEBPACK_IMPORTED_MODULE_1_ionic_angular__["j" /* Platform */], __WEBPACK_IMPORTED_MODULE_2__ionic_native_status_bar__["a" /* StatusBar */], __WEBPACK_IMPORTED_MODULE_3__ionic_native_splash_screen__["a" /* SplashScreen */]])
], MyApp);
